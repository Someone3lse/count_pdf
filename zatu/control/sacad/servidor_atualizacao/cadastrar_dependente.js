...
function salvarServidorDependente(){
  ...
  if (formValido) {
    projetouniversal.util.getjson({
      url: PORTAL_URL + "model/sacad/servidor_atualizacao/salvar_dependente",
      type: "POST",
...
function dependenteFormClean(obj){
  ...
  $(obj).find('input[name="dependente_id_old_s[]"]').val('0');
...
function depentendeFormOrdenar(){
  ...
  $($('form#form_servidor_dependente').find('div.box_dependente').get().reverse()).each(function (kDiv, vDiv){
    ...
    $(vDiv).find('input[name="dependente_id_old_s[]"]').attr('id', 'dependente_id_old_s'+(kDiv+1));
