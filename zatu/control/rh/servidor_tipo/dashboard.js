...
$(document).ready(function () {
  ...
  $('#form_servidor_tipo').submit(function () {
    window.onbeforeunload = null;
    projetouniversal.util.getjson({
      url: PORTAL_URL + "model/rh/servidor_tipo/salvar_servidor_tipo",
      type: "POST",
      data: $('#form_servidor_tipo').serialize(),
  ...
  function onSuccessSend(obj) {
    if (obj.msg == 'success') {
      postToURL(PORTAL_URL + 'view/rh/servidor_tipo/dashboard', {mensagem: obj.retorno});
  ...
  $("#btn_cancelar").click(function(){
    $('#id').val('');
    $('span#titulo_form').text(' NOVO TIPO DE SERVIDOR');
   ...
   swal.fire('Atenção', 'Este registro não pode ser exlcuido pois está vinculado a um contrato de servidor!', 'warning');
    ...
    // cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        ...
        projetouniversal.util.getjson({
          url: PPORTAL_URL + "model/rh/servidor_tipo/excluir_servidor_tipo",
          type: "POST",
          data: {id: id},
          enctype: 'multipart/form-data',
...
function editarRegistro(obj){
  $('span#titulo_form').text('EDIÇÃO DE TIPO DE SERVIDOR');
  $('span#btn_submit').text(' Atualizar');
  $("input#id").val($(obj).parents('tr').children('input#td_id').val());
  $("input#nome_st").val($(obj).parents('tr').children('#td_nome').text());
