...
$(document).ready(function () {
  ...
  $('#form_conferidor').submit(function () {
    window.onbeforeunload = null;
    projetouniversal.util.getjson({
      url: PORTAL_URL + "model/rh/conferidor/salvar_conferidor",
      type: "POST",
      data: $('#form_conferidor').serialize(),
  ...
  function onSuccessSend(obj) {
    if (obj.msg == 'success') {
      postToURL(PORTAL_URL + 'view/rh/conferidor/dashboard', {mensagem: obj.retorno});
  ...
  $("#btn_cancelar").click(function(){
    $('#id').val('');
    $('span#titulo_form').text(' NOVO VALIDADOR DE DADOS');
...
function carregaSetores(id){
  projetouniversal.util.getjson({
    url: PORTAL_URL + "model/rh/conferidor/get_setores_by_uo",
    type: "POST",
    data: {id: id},
    enctype: 'multipart/form-data',
  ...
  if ($(elem).attr('negado')) {
    ...
    // cancelButtonColor: '#d33',
      ...
      if (result.isConfirmed) {
        ...
        projetouniversal.util.getjson({
          url: PPORTAL_URL + "model/rh/conferidor/excluir_conferidor",
          type: "POST",
          data: {id: id},
          enctype: 'multipart/form-data',
...
function editarRegistro(obj){
  $('span#titulo_form').text('EDIÇÃO DE VALIDADOR DE DADOS');
