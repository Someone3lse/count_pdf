...
try {
  ...
  $rs = $db->prepare("SELECT u.senha, u.id, u.nome, u.email_institucional, u.foto, u.tel_celular, u.tel_fixo, u.status
    FROM seg_usuario u 
    WHERE u.login = :login");
    ...
    if($senha == $dadosUsuario['senha']) {
      if($dadosUsuario['status'] == 1) {
        ...
        //CRIAR AS SESSÕES DO USUARIO
          ...
          $_SESSION['tel_celular'] = $dadosUsuario['tel_celular'];
          $_SESSION['tel_fixo'] = $dadosUsuario['tel_fixo'];
        ...
        //STATUS ONLINE -> 1 - ONLINE e 2 - OFFLINE
          ...
          if ($fotoCut == "") {
            ...
            $_SESSION['foto_cut'] = str_replace($extensoes, 'cut.png', $fotoCut);
          }
        //ATUALIZANDO O STATUS ONLINE DO USUARIO
          $rs = $db->prepare("UPDATE seg_usuario SET online = '1' WHERE id = ?");
          $rs->bindValue(1, $id);
          $rs->execute();
        //ATUALIZANDO DADOS DA SESSÃO DO USUÁRIO
          ...
          $so = $separa[1];
          $rs3 = $db->prepare("SELECT seg_usuario_id
            FROM seg_sessao
            WHERE seg_usuario_id = ?");
          $rs3->bindValue(1, $_SESSION['zatu_id']);
          $rs3->execute();
          $qtd = $rs3->rowCount();
          if($qtd > 0) {
            $rs4 = $db->prepare("UPDATE seg_sessao 
            ...
            $rs4 = $db->prepare("INSERT INTO seg_sessao
              (host, ip, navegador, so, numero_sessao, dt_login, dt_atualizacao, seg_usuario_id)
              VALUES (?, ?, ?, ?, ?, NOW(), NOW(), ?)");
            $rs4->bindValue(1, $_SERVER["SERVER_NAME"]);
            $rs4->bindValue(2, $_SERVER['REMOTE_ADDR']);
            $rs4->bindValue(3, $browser . " " . $browser_version);
