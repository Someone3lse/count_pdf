...
try {
  ...
  if ($error == false) {
    if ($id != "" && $id != 0) {
      $stmt = $db->prepare('DELETE FROM rh_atestador WHERE seg_usuario_id_conferidor = ?;');
      $stmt->bindValue(1, $id);
      $stmt->execute();
      $db->commit();
      //MENSAGEM DE SUCESSO
      $msg['id'] = $id;
      $msg['msg'] = 'success';
      $msg['retorno'] = 'Validador de Dados excluido com sucesso.';
  ...
  $db->rollback();
  $msg['msg'] = 'error';
  $msg['retorno'] = "Erro ao tentar excluir o Validador de Dados: " . $e->getMessage();
  echo json_encode($msg);
  exit();
}
?>