...
$situacaoVacinaCovid  = strip_tags(@$_POST['situacao_vacina_covid']);
$obsVacinaCovid       = strip_tags(@$_POST['obs_vacina_covid']);
...
try {
  ...
  if (is_numeric($servAtualiProvaId) && $servAtualiProvaId != "" && $servAtualiProvaId != 0 ) {
    ...
    $stmt = $db->prepare('
      ...
      seg_usuario_id_conferidor = ? 
      WHERE id = ? ;');
    $stmt->bindValue(1, $conferenciaSituacao == 1 ? 6 : ($conferenciaSituacao == 2 ? 3 : 4));
    $stmt->bindValue(2, $status);
    $stmt->bindValue(3, $_SESSION['zatu_id']);
    $stmt->bindValue(4, $servAtualiId);
    $stmt->execute(); 
    ...
    if ($conferenciaSituacao == 1) {
      ...
      $stmt->bindValue(16, $rsServidorAtualizacao['eo_empregador_id']);
      $stmt->bindValue(17, $rsServidorAtualizacao['eo_setor_id']);
      $stmt->bindValue(18, $rsServidorAtualizacao['sangue_tipo']);
      $stmt->bindValue(19, $rsServidorAtualizacao['raca']);
      $stmt->bindValue(20, $rsServidorAtualizacao['enfermidade_portador']);
      $stmt->bindValue(21, $rsServidorAtualizacao['enfermidade_codigo_internacional']);
      $stmt->bindValue(22, $status);
      $stmt->bindValue(23, $_SESSION['zatu_id']);
      $stmt->bindValue(24, $servId);
      $stmt->execute(); 
