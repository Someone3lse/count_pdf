...
$idOld                        = $_POST['dependente_id_old_s'];
...
try {
      ...
      foreach ($id as $kId => $vId) {
        if (is_numeric($vId) && $vId != "" && $vId != 0 ) {
          $stmt = $db->prepare('
            ...
            SET
            sacad_servidor_atualizacao_id = ?, 
            sacad_servidor_dependente_id_old = ?, 
            codigo = ?, 
            nome = ?, 
            cpf = ?, 
          ...
          $stmt->bindValue(3, strip_tags($codigo[$kId]));
          $stmt->bindValue(4, strip_tags($nome[$kId]));
          $stmt->bindValue(5, strip_tags($cpf[$kId]));
          $stmt->bindValue(6, $ParentescoGrauId[$kId] != '' ? strip_tags($ParentescoGrauId[$kId]) : NULL);
          $stmt->bindValue(7, strip_tags($ParentescoGrauOutro[$kId]));
          $stmt->bindValue(8, formata_data(strip_tags($dtNascimento[$kId])));
          $stmt->bindValue(9, formata_data(strip_tags($dtCasamento[$kId])));
          ...
          if ($nome[$kId] != '' && $cpf[$kId] != '' && $ParentescoGrauId[$kId] != '') {
            $stmt = $db->prepare('
              INSERT INTO sacad_servidor_atualizacao_dependente 
              (sacad_servidor_atualizacao_id, 
                sacad_servidor_dependente_id_old, 
                codigo, 
                nome, 
  ...
  $db->rollback();
  $msg['msg'] = 'error';
  $msg['retorno'] = ''.var_dump($benefEndMunic).'Erro ao tentar salvar os dados de dependentes do Servidor:'. $e->getMessage();
  echo json_encode($msg);
  exit();
}
?>