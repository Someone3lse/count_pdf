...
<body class="hold-transition theme-primary bg-img login" style="background-image: url(http://localhost:80/zatu/assets/images/fundo_esquerdo.png) , url(http://localhost:80/zatu/assets/images/fundo_direito.png);">
  ...
  <div class="container h-p100 mt-60">
    <!-- <a href="<?= PORTAL_URL ?>login" class="back-login"><i class="fal fa-arrow-circle-left"></i> VOLTAR AO LOGIN</a> -->
            ...
            <div class="bg-white">
              <div class="p-20">
                ...
                <div class="text-center mb-5">
                  <br><h3 class="rh">Não consegue acessar o Sistema?</h3><br>
                </div>
                <div class="text-center mb-5">
                  Se você já possui cadastro em nosso portal e esqueceu seu nome de usuário ou sua senha, basta digitar seu e-mail e seguir as orientações enviadas para o seu e-mail.
                </div>
                <br/>
                <form id="form_esqueceu_senha" name="form_esqueceu_senha" method="post" action="#">
                  ...
                  <div class="row">
                    <div class="col-6 text-center">
                      <button type="reset" class="btn btn-info btn-login mt-10" onclick="window.location = '<?= PORTAL_URL ;?>login';"><strong>Voltar ao Login</strong></button>
                    </div>
                    <div class="col-6 text-center">
...
if(isset($_POST['email']) && @$_POST['email'] != '') {
  ...
  $stmt = $db->prepare("SELECT id, nome 
    FROM seg_usuario 
    WHERE email_pessoal = ? || email_institucional = ? AND status = 1 ");
  ...
  if($rsCount == 0) {
    ...
    $codigo = microtime();
    $stmt = $db->prepare("INSERT INTO seg_recupera_senha (email, codigo, dt_solicitacao, dt_alteracao, dt_expiracao, seg_usuario_id)
    ...
    Prefeitura de Tarauacá. <br><br> Envio de recuperação de senha para acesso ao Sistema ZATU. <br><br> Olá, '. $usuarioNome .'!<br><br> Você deve alterar sua senha no link: <a style="color: #00BBD3; text-transform: uppercase;" href="' . PORTAL_URL . 'redefinir_senha/' . $encode . '">clique aqui para alterar</a>
    </td>
    </tr>
    </tbody>
    <tfoot>
    ...
    if ($envio == 'sucesso') {
      ...
      <script type="text/javascript">
        ...
        setTimeout(function(){ window.location = "location.href='login'";}, 8000);
      </script>
      <?php
    } else {
      ?>
      <script type="text/javascript">
        window.onbeforeunload = null;
        $('#alert_email_erro').slideDown('slow');
        return false;
      </script>
      <?php
    }
