...
<head>
  ...
  <title>:: ZATU | Redefiniçao de Senha ::</title>
  <!-- FAVICON END -->
  <!-- CSS PLUGINS BEGIN -->
...
<body class="hold-transition theme-primary bg-img login" style="background-image: url(http://localhost:80/zatu/assets/images/fundo_esquerdo.png) , url(http://localhost:80/zatu/assets/images/fundo_direito.png);">
            ...
            <div class="bg-white">
              <div class="p-20">
                ...
                <div class="text-center mb-5">
                  <br><h3 class="rh">Redefinição de Senha</h3><br>
                </div>
                <form id="form_nova_senha" name="form_nova_senha" method="post" action="#">
                  ...
                  if (isset($_POST['login'])) {
                    ...
                    $codigo = $separa[1];
                    $stmt = $db->prepare("
                      SELECT ( dt_expiracao < NOW() ) AS menor 
                      FROM seg_recupera_senha 
                      WHERE email LIKE ? AND codigo LIKE ? AND dt_alteracao IS NULL");
                  ...
                  if ($login == '' && $rsCount == 0) {
                    ?>
                    <nav class="">Esse link já foi utilizado para alteração de senha. Caso seja necessário a alteração novamente da senha <a href="<?= PORTAL_URL;?>esqueceu_senha">clique aqui</a></nav>
                    <?php
                  } else if ($login == '' && $rsRecuperaSenha['menor'] == 1) {
                    ?>
                    <nav class=''>Seu código expirou. Caso seja necessário alterar a senha <br><a href="<?= PORTAL_URL;?>esqueceu_senha">clique aqui</a></nav>
                    <?php
                  } else {
                    ?>
                    <div class="text-center mb-5">
                      <h5>Para redefinir sua senha, basta informar sua senha antiga, sua nova senha e repetir sua nova senha.</h5>
                    </div>
                    <div id="div_nova_senha" class="form-group">
                      <div class="input-group mb-3">
                        <div class="input-group-prepend">
                          <span class="input-group-text bg-transparent"><i class="fal fa-lock-alt"></i></span>
                        </div>
                        <input type="password" class="form-control pl-15 bg-transparent" id="nova_senha" name="nova_senha" placeholder="Nova Senha" value="" required/>
                    ...
                    <div id="div_repete_nova_senha" class="form-group">
                      <div class="input-group mb-3">
                        <div class="input-group-prepend">
                          <span class="input-group-text bg-transparent"><i class="fal fa-lock-alt"></i></span>
                        </div>
                        <input type="password" class="form-control pl-15 bg-transparent" id="repete_nova_senha" name="repete_nova_senha" placeholder="Repita a Nova Senha" value="" required/>
                    ...
                    <div class="row"> 
                      <div class="col-6 text-center">
                        <button type="reset" class="btn btn-info btn-login mt-10" onclick="window.location = '<?= PORTAL_URL ;?>dashboard';"><strong>Voltar</strong></button>
                      </div>
                      <div class="col-6 text-center">
                        <button type="submit" class="btn btn-success btn-login mt-10"><strong>Salvar</strong></button>
                      </div>
                    </div>
                    <div id="alert_sucesso" class="alert alert-success" role="alert" style="display: none;">Sua senha foi redefinida com sucesso!</div>
  ...
  <script type="text/javascript" src="<?= JS_FOLDER; ?>redefinir_senha.js"></script>
